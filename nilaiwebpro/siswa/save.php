...
if (isset($_POST['save'])) {
    ...
    if (!file_exists(($_FILES['photo']['tmp_name'])) || !is_uploaded_file($_FILES['photo']['tmp_name'])) {
        ...
        if (in_array($ext, $extallowed)) {
            if ($size > 10000000) {
                echo "<script>alert('Ukuran file tidak boleh lebih dari 10 MB'); window.location='?ms=siswa&s=add';</script>";
            } else {
            ...
            echo "<script>alert('Akhiran file tidak sesuai'); window.location='?ms=siswa&s=add';</script>";
        }
    }
    ...
    if ($result) {
        ...
        echo "<script>alert('Data gagal disimpan'); window.location='?ms=siswa&s=add';</script>";
    }
} else {
    echo "Jangan langsung buka file ini. Tambah Data <a href='?ms=siswa&s=add'>Klik disini</a>";
}
